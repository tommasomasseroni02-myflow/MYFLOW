...
import { getSession, onAuthChange, signInWithEmail, signOutUser } from './auth.js';
...
artistForm?.addEventListener('submit', async (event) => {
  ...
  try {
    setStatus(artistStatus, 'Invio del magic link in corso...');
    if (profile.contact) {
      const { error } = await signInWithEmail(profile.contact);
    ...
    setStatus(
      artistStatus,
      'Profilo artista salvato. Controlla la tua email per completare il login.'
    );
    render();
    artistForm.reset();
...
providerForm?.addEventListener('submit', async (event) => {
  ...
  try {
    setStatus(providerStatus, 'Salvataggio del profilo e invio magic link...');
    if (profile.email) {
      const { error } = await signInWithEmail(profile.email);
      if (error) throw error;
    }
    await upsertProviderProfile(profile, session?.user?.id);
    setStatus(providerStatus, 'Profilo pubblicato! Controlla la mail per accedere.');
    providerForm.reset();
    await loadProviders();
    render();
  } catch (error) {
...
function updateSessionUI() {
  if (session?.user) {
    setStatus(sessionStatus, `Loggato come ${session.user.email || session.user.id}`);
  } else {
    setStatus(sessionStatus, 'Non sei autenticato. Usa i form per ottenere un link di accesso.');
  }
}
